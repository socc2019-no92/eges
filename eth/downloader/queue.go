...
type fetchResult struct {
	...
	//Geec
	GeecTxn 	types.Transactions
	Confirm 	*types.ConfirmBlockMsg
}
...
func (q *queue) DeliverBodies(id string, txLists [][]*types.Transaction, uncleLists [][]*types.Header, geectxnlist [][]*types.Transaction, confirmList []*types.ConfirmBlockMsg) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()
	log.Gdbug("deliver bodies")
	reconstruct := func(header *types.Header, index int, result *fetchResult) error {
		...
		result.Confirm = confirmList[index]
		result.GeecTxn = geectxnlist[index]
		return nil
	}
