...
type BlockChain struct {
	...
	GeecbodyRLPCache *lru.Cache     // Cache for the most recent block bodies in RLP encoded format
...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	...
	GeecbodyRLPCache, _ := lru.New(bodyCacheLimit)
	...
	bc := &BlockChain{
		...
		GeecbodyRLPCache:GeecbodyRLPCache,
	...
	//geec code
	bc.geecState = new(GeecState)
	bc.geecState.Init(bc, db, engine.GetEthBase())




	var err error
	bc.hc, err = NewHeaderChain(db, chainConfig, engine, bc.getProcInterrupt, bc.geecState)
	if err != nil {
		return nil, err
	}
...
func (bc *BlockChain) SetGeecStatePM(pm interface{}){
	bc.geecState.SetPm(pm)
}
...
func (bc *BlockChain) insert(block *types.Block) {
	log.Gdbug("Insert Called", "BlockNum", block.NumberU64());

	//thw, ret := bc.engine.(*geec.TrustedHW)
	//if ret{
	...
	thws := bc.hc.GetGeecState().(*GeecState)
	thws.NotifyNewBlock(block)
}
...
//xs: only used for repliy blocks
...
//
//func (bc *BlockChain) GetGeecBodyRLP (hash common.Hash) rlp.RawValue {
//	// Short circuit if the body's already in the cache, retrieve otherwise
//	if cached, ok := bc.GeecbodyRLPCache.Get(hash); ok {
//		return cached.(rlp.RawValue)
//	}
//	body := GetBodyRLP(bc.db, hash, bc.hc.GetBlockNumber(hash))
//	if len(body) == 0 {
...
//	}
//	// Cache the found body for next time and return
//	bc.GeecbodyRLPCache.Add(hash, body)
//	return body
//}
...
func (bc *BlockChain) WriteBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	log.Geec("Write block with state", "Num", block.Number().Uint64(), "author",block.Coinbase(), "hash", block.Hash())
...
//xs: insert a block to the chain
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	// Do a sanity check that the provided chain is actually ordered and linked
	log.Debug("Insert chain", "start", chain[0].NumberU64(), "end", chain[len(chain)-1].NumberU64())
	...
	for i, block := range chain {
		...
		if err == nil {
			...
			log.Info("Valdiate error", "error", err)
		}
		...
		if err != nil {
			log.Warn("WriteBlockWithState Err", err)
			return i, events, coalescedLogs, err
		}
		switch status {
		case CanonStatTy:
...
const statsReportLimit = 0 * time.Second
...
func (st *insertStats) report(chain []*types.Block, index int, cache common.StorageSize) {
	...
	if index == len(chain)-1 || elapsed >= statsReportLimit {
		...
		log.Gdbug("Imported new chain segment", context...)
...
func (bc *BlockChain) GetGeecState() geecCore.State {
	return bc.geecState
}