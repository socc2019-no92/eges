...
def start_nodes():
    ...
    for i in range(boostrap_node_count+normal_node_count):
        ...
        if i == 0:
            start_node_cmd = ["ssh", "-f", "%s" % machine, "cd %s && /usr/bin/nohup ./build/bin/geth --datadir data/%s --unlock %s --password ./pass %s --verbosity %d --networkid 930412 --ipcdisable --port 619%s --rpc --rpccorsdomain \"*\" --rpcport 81%s --bootnodes %s --consensusPort \"100%s\" --consensusIP %s  --syncmode \"full\" --maxpeers %d --mine --geecTxnPort 3333 1>>data/logs/%s 2>&1 &" % (code_path, node, accounts[i], debug, verbos, node, node, bn_addr, node, machine,  max_peer, node)]
        else:
            if i == 3: 
                time.sleep(20)
            start_node_cmd = ["ssh", "-f", "%s" % machine, "cd %s && /usr/bin/nohup ./build/bin/geth --datadir data/%s --unlock %s --password ./pass %s --verbosity %d --networkid 930412 --ipcdisable --port 619%s --rpc --rpccorsdomain \"*\" --rpcport 81%s --bootnodes %s --consensusPort \"100%s\" --consensusIP %s  --syncmode \"full\" --maxpeers %d --mine 1>>data/logs/%s 2>&1 &" % (code_path, node, accounts[i], debug, verbos, node, node, bn_addr, node, machine,  max_peer, node)]
