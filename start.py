...
def start_nodes():
    ...
    nCandidates = config['parameters']['nCandidates']
    nAcceptors = config['parameters']['nAcceptors']
    blockTimeout = config['parameters']['blockTimeout']
    txnPerBlock = config['parameters']['txnPerBlock']
    txnSize = config['parameters']['txnSize']
    breakdown = config['parameters']['breakdown']
    failureTest = config['parameters']['failureTest']
    totalNodes = int(boostrap_node_count + normal_node_count)
    
    if failureTest == True:
        failureFlag = "--failureTest"
    else:
        failureFlag = ""
    ...
    for i in range(boostrap_node_count+normal_node_count):
        ...
        if i == 0:
            start_node_cmd = ["ssh", "-f", "%s" % machine, "cd %s && /usr/bin/nohup ./build/bin/geth --datadir data/%s --unlock %s --password ./pass %s --verbosity %d --networkid 930412 --ipcdisable --port 619%s --rpc --rpccorsdomain \"*\" --rpcport 81%s --bootnodes %s --consensusPort \"100%s\" --consensusIP %s  --syncmode \"full\" --maxpeers %d --nCandidates %d --nAcceptors %d --blockTimeout %d --txnPerBlock %d --txnSize %d --totalNodes %d --mine %s %s --geecTxnPort 3333 1>>data/logs/%s 2>&1 &" % (code_path, node, accounts[i], debug, verbos, node, node, bn_addr, node, machine,  max_peer, nCandidates, nAcceptors, blockTimeout, txnPerBlock, txnSize, totalNodes, breakdownFlag, failureFlag, node)]
        else:
            start_node_cmd = ["ssh", "-f", "%s" % machine, "cd %s && /usr/bin/nohup ./build/bin/geth --datadir data/%s --unlock %s --password ./pass %s --verbosity %d --networkid 930412 --ipcdisable --port 619%s --rpc --rpccorsdomain \"*\" --rpcport 81%s --bootnodes %s --consensusPort \"100%s\" --consensusIP %s  --syncmode \"full\" --maxpeers %d --nCandidates %d --nAcceptors %d --blockTimeout %d --txnPerBlock %d --txnSize %d --totalNodes %d --mine %s %s 1>>data/logs/%s 2>&1 &" % (code_path, node, accounts[i], debug, verbos, node, node, bn_addr, node, machine,  max_peer, nCandidates, nAcceptors, blockTimeout, txnPerBlock, txnSize, totalNodes, breakdownFlag, failureFlag, node)]
...
def prepare():
    for machine in machines:
        ...
        cmd = ["ssh", "%s" % machine, "cd %s && rm -rf data/*; rm -rf /tmp/geec.sock; pkill geth; pkill -9 geth; mkdir -p data/logs " % code_path]
        print(cmd)
        subprocess.run(cmd)
...
def generate_json():
    ...
    # normal_node_count = config['normal_nodes'] 
    # total_node = boostrap_node_count+ normal_node_count
    # committee_size = config['parameters']['committee_ratio']
    # avg_blks_interval = int(total_node)


    # genesis['config']['thw']['initial_TTL'] = avg_blks_interval 
    # genesis['config']['thw']['bonus_TTL'] = avg_blks_interval * 2
    # genesis['config']['thw']['renew_TTL'] = 20
    # genesis['config']['thw']['max_TTL'] = avg_blks_interval * 4
    # genesis['config']['thw']['TTL_interval'] = 10
